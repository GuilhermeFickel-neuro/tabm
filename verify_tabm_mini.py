...
def verify_tabm_mini_implementation():
    ...
    try:
        ...
        # Check that bins are computed
        assert model.bins is not None, "Bins should not be None for piecewise-linear embeddings"
        print(f"   ✅ Bins computed: {len(model.bins)} bins")
        ...
        with torch.no_grad():
            ...
            expected_embed_shape = (16, n_features * 16)  # n_features * d_embedding
        ...
        print(f"   • Bins: {len(model.bins)} computed from training data")
